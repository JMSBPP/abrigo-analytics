...
description: NON-NEGOTIABLE — every write to CLAUDE.md, contracts/docs/superpowers/plans/, contracts/docs/superpowers/specs/, or any tracked contracts/docs/** artifact runs two-wave verification (Reality Checker mandatory + specialized agent matched to document purpose) BEFORE commit
type: feedback
originSessionId: d1cfac41-85eb-4cae-ae40-bddd97fffc23
---
NON-NEGOTIABLE: Every write to a project document — `.worktree/ranFromAngstrom/CLAUDE.md`, anything under `contracts/docs/superpowers/plans/`, anything under `contracts/docs/superpowers/specs/`, or any tracked `contracts/docs/**` artifact — runs a two-wave verification BEFORE the change is committed.
...
- Out of scope for this rule: memory files (`~/.claude/.../memory/`), scratch artifacts (`contracts/.scratch/`), runtime outputs (notebooks, pipeline data, parquets).
