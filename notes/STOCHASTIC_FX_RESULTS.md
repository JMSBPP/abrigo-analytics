...
**Hardware-constraint disclosure (FLAG-RC-V0.5-1).** Phase C reference for the Merton family was sampled at hardware-constrained `N_REF = 5_000` during this Task 6 driver execution. The production constant `N_REF = 100_000` is unchanged in spec v0.5 §11.7 and in `simulations/stochastic_fx/variance_proxy.py`; the spec-pinned constant defines production behavior, and this Task 6 run is a hardware-constrained local execution of the same code path. The monkey-patch pattern matches the autouse fixture already in `simulations/tests/test_saas_stochastic_fx.py::TestPhaseCMertonEmpiricalCDF` (Task 4.2 review-passed). Disclosure surface: this section, `scratch/2026-05-11-stochastic-fx-execution/driver.py` docstring, `scratch/2026-05-11-stochastic-fx-execution/moment_verification_log.md`, `scratch/2026-05-11-stochastic-fx-execution/ks_test_log.md`, and the commit message — five mutually-corroborating disclosure points per FLAG-RC-V0.5-1 disposition.
